...
A small C program that converts lowercase letters to uppercase and vice versa.

## 🧩 Description

The source file **charconverter.c** implements the logic, while **main.o** contains the main entry point.
...
```bash
gcc -I. -std=c99 charconverter.c -x c main.o -o charconverter
```
...
|:------|:----------------|:-------------|
| `a` | `A` | `a` is a lowercase letter so it's converted to uppercase. |
| `A` | `a` | `A` is a uppercase letter so it's converted to lowercase. |
| `1` | `1` | `1` is a number so it's not converted. |
| `$` | `$` | `$` is a special character so it's not converted. |
